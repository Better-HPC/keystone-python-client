...
class Create(IsolatedAsyncioTestCase):
    ...
    async def test_record_data_matches_request(self) -> None:
    ...
    async def test_error_on_failure(self) -> None:
        """Test an error is raised when record creation fails."""

        with self.assertRaises(httpx.HTTPError):
            await self.client.create_cluster()


class Retrieve(IsolatedAsyncioTestCase):
    ...
    async def test_retrieve_by_pk(self) -> None:
        """Test the retrieval of a specific record via its primary key."""

        pk = self.test_cluster['id']
        retrieved_cluster = await self.client.retrieve_cluster(pk=pk)
    ...
    async def test_retrieve_with_filters(self) -> None:
        ...
        retrieved_clusters = await self.client.retrieve_cluster(filters={"name": "Test-Cluster"})
    ...
    async def test_retrieve_all(self) -> None:
        ...
        all_clusters = await self.client.retrieve_cluster()
        self.assertIsInstance(all_clusters, list)
        self.assertGreater(len(all_clusters), 0)

    async def test_none_on_missing_record(self) -> None:
        """Test `None` is returned when a record does not exist."""

        missing_cluster = await self.client.retrieve_cluster(pk=999999)
...
class Update(IsolatedAsyncioTestCase):
    ...
    async def test_update_record(self) -> None:
        ...
        updated_record = await self.client.update_cluster(pk=pk, data=updated_data)
        self.assertIsNotNone(updated_record)
        self.assertEqual(updated_record['description'], "Updated description")

    async def test_update_nonexistent_record(self) -> None:
        """Test updating a nonexistent record raises an error."""

        with self.assertRaises(httpx.HTTPError):
            await self.client.update_cluster(pk=999999, data={'description': "This should fail"})
    ...
    async def test_no_update_on_empty_data(self) -> None:
        """Test an empty update request does not change the record."""

        pk = self.test_cluster['id']
        original_data = self.test_cluster.copy()
        updated_record = await self.client.update_cluster(pk=pk, data={})
        self.assertEqual(updated_record, original_data)


class Delete(IsolatedAsyncioTestCase):
    ...
    async def test_delete_record(self) -> None:
        ...
        pk = self.test_cluster['id']
        await self.client.delete_cluster(pk=pk)

        retrieved_cluster = await self.client.retrieve_cluster(pk=pk)
        self.assertIsNone(retrieved_cluster)

    async def test_delete_nonexistent_record_silent(self) -> None:
        """Test deleting a nonexistent record exits silently."""

        await self.client.delete_cluster(pk=999999)

    async def test_delete_nonexistent_record_raise(self) -> None:
        """Test deleting a nonexistent record raises an exception when specified."""

        with self.assertRaises(httpx.HTTPError):
            await self.client.delete_cluster(pk=999999, raise_not_exists=True)
